...
<!-- ============================================================
     GIF #1 — Hero (메인 하이라이트)
     ────────────────────────────────────────────────────────────
     내용: 보스전 전체 흐름을 10~15초로 압축.
           이동 → 공격 → 회피 → 저스트 회피(슬로우) → 차지 스킬 → 보스 처치
     촬영 팁: Rapier 캐릭터로 중반 보스(Berserker 추천). 세로 화면 그대로.
     크기: 가로 360px 권장 (세로 화면이라 너무 넓으면 어색)
     ============================================================ -->
<p align="center">
  <img src="docs/gifs/hero_gameplay.gif" width="360" alt="Rapier gameplay highlight">
</p>
...
<!-- ============================================================
     GIF #2 — Combat Basics (기본 전투 조작)
     ────────────────────────────────────────────────────────────
     내용: 4가지 제스처를 순서대로 시연.
           Drag 이동 → Tap 공격(인디케이터 표시) → Swipe 회피 대시 → Hold 차지 게이지 → Release 스킬 발동
     촬영 팁: 일반 적 또는 Titan(1번 보스)상대. 각 입력 사이 1~2초 여유. 8~12초.
     ============================================================ -->
<p align="center">
  <img src="docs/gifs/combat_basics.gif" width="360" alt="Four gesture inputs — tap, swipe, drag, hold">
</p>
...
<!-- ============================================================
     GIF #3 — Just Dodge (저스트 회피 클로즈업)
     ────────────────────────────────────────────────────────────
     내용: 보스 공격 예고(인디케이터 스캔라인) → 스와이프 회피 →
           저스트 회피 발동(슬로우 + 줌인) → 고유 스킬 즉시 사용
     촬영 팁: Rapier 캐릭터의 표식 대시가 가장 시각적. 5~8초.
              슬로우 구간이 핵심이므로 여기서 잘리지 않게.
     ============================================================ -->
<p align="center">
  <img src="docs/gifs/just_dodge.gif" width="360" alt="Just Dodge — slow motion and unique skill activation">
</p>
...
<!-- ============================================================
     GIF #4 — Character Showcase (캐릭터별 고유 스킬 모음)
     ────────────────────────────────────────────────────────────
     내용: 4캐릭터 고유 스킬을 2~3초씩 이어 붙인 몽타주.
           Rapier 표식 폭발 → Warrior 패링+대지분쇄 →
           Assassin 잔상 난무 → Ranger 지뢰+관통 화살
     촬영 팁: 각 캐릭터 차지 스킬 발동 순간 위주. 8~12초.
              아직 미구현 캐릭터가 있다면 구현된 것만으로 구성.
     ============================================================ -->
<p align="center">
  <img src="docs/gifs/characters.gif" width="360" alt="Four characters — unique skill showcase">
</p>
...
<!-- ============================================================
     GIF #5 — Boss Fight (보스전 하이라이트)
     ────────────────────────────────────────────────────────────
     내용: 인상적인 보스 1~2종의 전투 장면.
           공격 예고 인디케이터(부채꼴/사각형) 표시 → 회피 → 반격 흐름.
     촬영 팁: Twin Phantoms(2체 동시) 또는 Stormcaller(다방향 공격)가 시각적으로 화려.
              보스 페이즈 전환 장면이 들어가면 더 좋음. 8~12초.
     ============================================================ -->
<p align="center">
  <img src="docs/gifs/boss_fight.gif" width="360" alt="Boss battle — attack indicators and phase transition">
</p>
...
<!-- ============================================================
     GIF #6 — UI Flow (로비 → 스테이지 → 클리어 → 장비 획득)
     ────────────────────────────────────────────────────────────
     내용: 게임 전체 루프를 UI 중심으로 빠르게.
           로비 화면 → 스테이지 선택 → 전투 진입 → 클리어 보상 → 장비 장착
     촬영 팁: 각 화면 1~2초씩, 전환 자연스럽게. 가챠 연출이 있으면 포함. 10~15초.
     ============================================================ -->
<p align="center">
  <img src="docs/gifs/ui_flow.gif" width="360" alt="Game loop — lobby, stage, clear, equipment">
</p>
