...
public class DocSyncTool : EditorWindow
{
    ...
    private static string GetDesignDocTemplate()
    {
        return
@"# RAPIER 프로토타입 기획서
**Prototype Design Document v1.3.0**
2026-03-10
...
| Tap | Attack | 이동 거리 < 20px, 지속 < 0.2초 | 전방 좁은 범위 광역 공격. 0.5초 딜레이. 딜레이 중 연타 차단 |
| Swipe | Dodge | 이동 거리 ≥ 20px, 지속 < 0.3초 | 방향 회피. 무적 0.2초. 쿨다운 2초. 쿨다운 중 시도 시 차단 |
...
| 회피 무적 시간 | 0.2초 | unscaledTime 기반. 저스트 회피는 타이밍으로 판정 |
...
| 구분 | HP | 공격력 | 이동속도 | 비고 |
|------|----|--------|----------|------|
| 플레이어 | 500 | 50 (좁은 범위 광역) | 5.0 | 회피 무적 0.2초, 쿨다운 2초 |
...
| Phase 7 | 캐릭터 고유 메커니즘 1종 (Rapier) | 표식 시스템 + 대시 스킬 + 차지 스킬 동작 확인 |
