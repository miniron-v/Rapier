...
// 역할: PROJECT_GUIDELINES.md 의 내용을 캐시로 보유하며,
//       Tools > Write Project Guidelines 메뉴로 md 파일을 (재)생성합니다.
//
// ⚠️  주의: GetCachedContent() 의 캐시 영역은
//     GuidelinesSync.cs 가 재컴파일 시 자동으로 md 내용과 동기화합니다.
//     이 영역을 수동으로 편집하지 마세요.
//     지침서를 수정하려면 PROJECT_GUIDELINES.md 를 직접 편집하세요.
// =============================================================
using UnityEditor;
using UnityEngine;
using System.IO;
...
namespace Game.Core.Editor
{
    public static class GuidelinesCreator
    {
        ...
        public static void WriteGuidelines()
        {
            ...
            string fullPath = ToFullPath(OUTPUT_PATH);
            Directory.CreateDirectory(Path.GetDirectoryName(fullPath));
            File.WriteAllText(fullPath, content, Encoding.UTF8);
            AssetDatabase.Refresh();
            Debug.Log($"[GuidelinesCreator] 지침서 파일 생성 완료: {OUTPUT_PATH}");
            Selection.activeObject = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(OUTPUT_PATH);
        }

        /// <summary>
        /// GuidelinesSync.cs 가 자동 갱신하는 캐시 영역입니다.
        /// 지침서를 수정하려면 PROJECT_GUIDELINES.md 를 직접 편집하세요.
        /// </summary>
