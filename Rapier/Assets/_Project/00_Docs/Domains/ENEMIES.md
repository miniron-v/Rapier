...
└── BossPresenterBase (abstract)    ← 가변 페이즈 (1~3+), 페이즈별 시퀀스 교체
    ├── TitanBossPresenter          ← 구현 완료
    ├── SpecterBossPresenter        ← 골격만 존재, 내부 구현 미완
    ├── PyromancerBossPresenter     ← 미구현
    ├── BerserkerBossPresenter      ← 미구현
    ├── StormcallerBossPresenter    ← 미구현 (3페이즈 보스)
    ├── GravekeeperBossPresenter    ← 미구현
    └── TwinPhantomsBossPresenter   ← 미구현 (1페이즈, 2체 동시)
```
...
| 2 | Specter | 2 | Teleport, Melee | 골격만 |
| 3 | Pyromancer | 2 | Projectile, GroundHazard, MultiDirectional | 미구현 |
| 4 | Berserker | 2 | Melee×3~4 콤보, Charge | 미구현 |
| 5 | Stormcaller | 3 | MultiDirectional, Projectile (유도) | 미구현 |
| 6 | Gravekeeper | 2 | Summon, Melee | 미구현 |
| 7 | Twin Phantoms | 1 | Melee × 2 (본체+분신 협공) | 미구현 |
