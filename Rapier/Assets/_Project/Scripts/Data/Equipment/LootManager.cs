...
namespace Game.Data.Equipment
{
    ...
    {
        private const int MAX_DROP_COUNT = 2;
        ...
        /// - 최대 2개 제한: 상위 등급(Unique → Epic → Rare → Normal) 우선, 초과분 버림.<br/>
        ...
        public List<EquipmentInstance> RollDrop(DropTableData dropTable)
        {
            ...
            // 최대 2개 제한 (이미 상위 등급 순으로 정렬되어 있음)
            int take = Mathf.Min(candidates.Count, MAX_DROP_COUNT);
