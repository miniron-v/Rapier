...
- **성공**: 모달 위 풀스크린 플래시 (등급 색, alpha 0.5 → 0, 0.4초). Image 5개 파편 — 중앙에서 360° 분산, scale 1→0 + alpha 1→0 (0.6초). 토스트 "강화 성공!" 1.5초. 이후 데이터 자동 갱신 (다음 단계 정보로 표시 전환).
- **실패**: 모달 RectTransform 좌우 흔들림 (±10px, 0.3초). 회색 플래시 (alpha 0.4 → 0, 0.3초). 토스트 "강화 실패..." 1.5초. 정보는 단계 변화 없으므로 가루/확률 갱신만.
