...
- 저스트 회피 후 고유 스킬: 가장 가까운 적에게 대시 → 표식 1중첩 부여 + 데미지 → 복귀. 표식 최대 5중첩.
...
- `GestureRecognizer.TriggerJustDodge()`가 유일한 발동 API.
...
- AnimationCurve 끝값은 0보다 큰 값(0.50f 이상) 유지 — 0이면 while 루프 무한 반복 위험.
