...
- **참조 경로**: `EquipmentManager` 는 순수 C# 클래스라 Inspector 주입 불가 → `GameBootstrap` 이 `Resources.Load<EquipmentDatabase>("EquipmentDatabase")` 로 로드 후 `EquipmentManager.Init(saveProvider, database)` 로 주입. `EquipmentManager` 는 생성자 대신 `Init` 파라미터에 `EquipmentDatabase database` 인자를 추가 받아 private 필드에 보관.
- **신규 SO 추가 시 규칙**: 새 `EquipmentItemData` / `RuneItemData` 를 만들면 반드시 `EquipmentDatabase` 배열에 수동 등록. (자동 수집 없음 — 프로젝트 컨벤션.)
- **에셋 위치**: `Assets/_Project/Resources/EquipmentDatabase.asset` (신규 Resources 폴더). SO 클래스 파일은 `Assets/_Project/Scripts/Data/Equipment/EquipmentDatabase.cs` 에 위치.
- **로드 실패 방어**: `Resources.Load` 결과가 `null` 이면 `Debug.LogWarning` 후 `EquipmentManager` 는 빈 DB 를 가지고 진행한다 (Deserialize 가 전부 누락 처리되어 경고만 찍힘 — 게임 기동은 막지 않음).
...
Phase 13-B 의 `[RuntimeInitializeOnLoadMethod(BeforeSceneLoad)]` 흐름에 `EquipmentDatabase` 로드 단계만 추가:

```
1. (신규) var db = Resources.Load<EquipmentDatabase>("EquipmentDatabase");
2. var em = new EquipmentManager();
3. em.Init(saveProvider: null, database: db);          ← Init 시그니처 확장
4. var sm = new SaveManager();
5. sm.SetEquipmentProvider(em);
6. sm.Load()
   → save.json 존재 시: SaveData.ownedEquipment + equippedMap 을
     IEquipmentSaveProvider 경로로 DeserializeOwnedEquipment → DeserializeEquippedMap 순 호출
   → save.json 없음: 아무것도 하지 않음 (빈 인벤토리 + SaveManager.Save() 로 최초 파일 생성)
7. ServiceLocator.Register(sm);
8. 씬 로드
```

**순서 제약**: `DeserializeOwnedEquipment` 를 `DeserializeEquippedMap` 보다 먼저 호출해야 함. 장착 맵 복원이 `instanceId` 로 ownedEquipment 를 참조하기 때문. `SaveManager.Load()` 내부 호출 순서가 이를 보장해야 한다.
