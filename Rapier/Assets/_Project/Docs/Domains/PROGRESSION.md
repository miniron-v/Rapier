...
| `GameBootstrap` | 앱 전역 서비스 진입점. Lobby 씬 루트에 배치되며 `SaveManager`/`EquipmentManager` 인스턴스를 생성·배선·등록한다 |

### 라이프사이클 (Phase 13-B)

`SaveManager` 와 `EquipmentManager` 는 POCO 라 MonoBehaviour 자가 등록을 쓸 수 없다. 앱 전역 서비스 진입점 `GameBootstrap : MonoBehaviour` 가 두 인스턴스의 생성·배선·등록을 책임진다.

- **생성 주체**: `GameBootstrap` (Lobby 씬 루트에 배치).
- **생성 시점**: Lobby 씬 진입 시 `Awake()` 에서 1회. 다음 배선 순서를 지킨다.
  1. 중복 가드 — `ServiceLocator.Get<SaveManager>()` 이미 존재하면 `Destroy(gameObject)` 후 return (재진입 방지).
  ...
  7. **파일이 없었던 경우 즉시 `sm.Save()` 1회 호출** — 최초 `save.json` 을 생성하여 첫 실행 증거와 메타 필드(deviceId/schemaCreatedAt) 를 디스크에 고정.
  8. `ServiceLocator.Register(sm);`
- **지속성**: `DontDestroyOnLoad(gameObject)` 로 Lobby ↔ Stage/BossRush 씬 전환 간 단일 인스턴스 유지. 인스턴스 교체는 일어나지 않는다.
- **접근**: 어디서든 `ServiceLocator.Get<SaveManager>()` / `ServiceLocator.Get<EquipmentManager>()` 로 조회.
- **파괴**: 앱 종료까지 유지. 명시적 `Dispose` 없음.
- **배선 누락 감지**: `LobbyPresenter.Init` 는 `ServiceLocator.Get<SaveManager>()` 결과를 사용한다. `GameBootstrap` 이 Lobby 씬에 없으면 `SaveManager` 가 null 이므로 `HomeTabPresenter` 등이 폴백으로 진입한다 — 이 경우 **에디터 경고 로그** 를 남겨 배선 누락을 조기에 드러낸다.
