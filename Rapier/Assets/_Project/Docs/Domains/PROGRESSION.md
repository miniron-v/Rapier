...
PlayerPrefs 금지. JSON 파일을 `Application.persistentDataPath` 하위에 저장.
...
| 재화 | 골드, 가챠 티켓, 강화 재료 등 모든 재화 | 재화 변동 시 |
...
| 객체 | 책임 |
|------|------|
| `SaveDataModel` | 저장 가능한 모든 데이터의 직렬화 모델 |
| `SaveDataRepository` | JSON 직렬화/역직렬화, 파일 I/O |
| `SaveService` | 저장/로드 호출 진입점, 디바운스/배치 처리 |
