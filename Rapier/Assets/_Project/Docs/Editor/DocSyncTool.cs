...
{
    private const string DocsPath = "Assets/_Project/Docs";
...
- **차지 스킬 (홀드 → 릴리즈)**: 표식이 있는 모든 적에게 ATK × (chargeSkillPercent/100) × stacks × SkillDmgMult 데미지. 이후 모든 표식 소비
...
| Swipe | Dodge | 이동 거리 ≥ 20px, 지속 < 0.3초 | 입력 벡터 방향 회피. 대시 전 구간 무적. 쿨다운 2초 |
...
│
└── _Project/                     # 공개 저장소 대상
    ├── Docs/                     # 개발 문서, 지침서 (Editor/ 일회성 툴 포함)
    ├── Scripts/
    │   ├── Core/                 # Camera, Interfaces, Scene, Services, Stage
    │   ├── Input/                # GestureRecognizer, InputState
    │   ├── Combat/               # IDamageable
